...
class administrateurController extends MainController
{
   ...
   {
       if($this->administrateurManager->isCombinaisonValide($login,$password)){
           if($this->administrateurManager->isCompteActive($login)){
               ToolBox::ajouterMessageAlerte("Bienvenu sur la page Administrateur  ".$login."  !", ToolBox::COULEUR_VERTE);
               //Variable de session
               $_SESSION['profil'] = [
                   "login" => $login
               ];
               ...
               header("location: ".URL."profilAdmin");
           } else {
               $msg = "Le compte ".$login. " n'a pas de droit d'administrateur.  ";
   ...
   public function profilAdministrateur()
    {
        $datas = $this->administrateurManager->getUserInformation($_SESSION['profil']['login']);
        ...
        $data_page = [
            ...
            "page_title" => "Page profil Admin",
            "utilisateurs" => $utilisateurs,
            "utilisateur" => $datas,
            "page_javascript" => ["main.js","script.js"],
            //"page_javascript" => "main.js",
            //"page_css" => "style.css",
            "page_css" => "main.css",
            //"page_css" => ["main.css","style.css"],
            "view" => "views/Administrateur/administrateur.view.php",
    ...
    public function loginAdministrateur()
    {
        $data_page = [
            ...
            "page_css" => "main.css",
            //"page_css" => ["main.css","style.css"],
            "view" => "views/Administrateur/administrateurLogin.view.php",
            "template" => "views/common/template.php"
        ];
        $this->genererPage($data_page);
    }
    ...
    public function validation_creerCompte($login,$password,$mail)
    {
        ...
        if($this->administrateurManager->checkLogin($login)){
            $passwordCrypte = password_hash($password,PASSWORD_DEFAULT);
            $clef = rand(0,9999);
            if($this->administrateurManager->creatCompteInBdd($login,$passwordCrypte,$mail,$clef,"profils/avatar.jpg",'utilisateur')){
    ...
    {
        $utilisateur = $this->administrateurManager->getUserInformation($login);
    ...
    {
        if($this->administrateurManager->bddValidationCompteMail($login,$clef)){
    ...
    {
        if($this->administrateurManager->updateMail($_SESSION['profil']['login'],$mail)){
    ...
    public function modificationPassword()
    {
        $data_page = [
            ...
            "page_css" => "main.css",
            //"page_css" => ["main.css","style.css"],
            "view" => "views/Utilisateur/administrateurModifPassword.view.php",
            "template" => "views/common/template.php"
        ];
        $this->genererPage($data_page);
    }
    ...
    public function Validation_ModificationPassword($ancienPassword,$nouveauPassword,$confirmPassword)
    {
        if($nouveauPassword === $confirmPassword){
            if($this->administrateurManager->isCombinaisonValide($_SESSION['profil']['login'],$ancienPassword)){
                $passwordCrypte = password_hash($nouveauPassword,PASSWORD_DEFAULT);
                if($this->administrateurManager->updatePassword($_SESSION['profil']['login'],$passwordCrypte)){
    ...
    {
        if($this->administrateurManager->bdSuppressionCompte($_SESSION['profil']['login'])){
